...
$sqldelete = "DELETE FROM trainer WHERE emp_ID='$empid'";//delete statement
...
$sql = "SELECT * FROM trainer";// embed a select statement in php
...
if($result->num_rows > 0){// check for number of rows. If there are records, build a table to show them
 echo "<table style='border: solid 1px black;'>
	<tr style='border: solid 1px black;'>
	    ...
	    <th style='border: solid 1px black;'>Hourly Training Rate</th>
...
while ($row = $result -> fetch_assoc()){// Fetch the query result and store them in an array
	echo '<tr style="border: solid 1px black;">
		...
		<td style="border: solid 1px black;">'.$row['hourlyTrainingRate'].'</td>
...
<!--For each row of the table, we create a hyperlink and including the parameter sno to be used it in the destination page (edit.php)-->
		<td style="border: solid 1px black;"> <a href="edittrn.php?empid='.$row['emp_ID'].'">Click </a></td>
		<td> <a href="trainer.php?empid='.$row['emp_ID'].'&mode=delete">Delete </a></td>
	      </tr>';
}
 echo "</table>";
...
if(isset($_POST['insert'])){ //things to do, once the "submit" key is hit
	...
	$hrate = $_POST[hrate_tb];//get form values sno attribute
	...
	// Create connection
	$conn = new mysqli($servername, $username, $password, $dbname);
	$sql = "INSERT INTO trainer VALUES ('$id', '$fname', '$lname','$section','$hrate')";//embed insert statement in PHP
...
To add a trainer, enter their information below:
<br/> <br/>
...
<br/> <br/>
Hourly Rate : <input type ="text" name ="hrate_tb"/>
<br/> <br/>
